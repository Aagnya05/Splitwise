...
def delete_person(person_id: int, db: Session = Depends(get_db)):
    ...
    # Note: if a person is referenced in expenses, the FK is RESTRICT.
    # You can either:
    #  - prevent delete, or
    #  - cascade manually.
    # For now, prevent delete if used in expenses.
    used_as_payer = db.query(models.Expense).filter(models.Expense.paid_by == person_id).first()
    used_as_participant = db.query(models.ExpenseParticipant).filter(models.ExpenseParticipant.person_id == person_id).first()
